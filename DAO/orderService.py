...
class OrderService(DBConnection):
    def create_order(self,customerid):
        ...
        orderid = self.cursor.execute("select top 1 OrderID from Orders order by OrderID desc ").fetchone()[0] 
        self.cursor.execute("INSERT INTO Orders VALUES (?,?, ?, ?,?)",
                           (orderid,customerid, date.today(), total_amount,"Pending"))
        self.conn.commit()
        for item in order_items:
                orderdetailid = self.cursor.execute("select top 1 OrderDetailID from OrderDetails order by OrderDetailID desc ").fetchone()[0]
